...
- 추상클래스와 인터페이스를 둘다 구현할 시 추상클래스의 구현이 우선
...
## 질문
- 캡슐화에서 public으로 지정된 getter, setter 를 이용하면 어차피 private 지정된 변수에 접근가능한데 사용하는 이유?- 	 
    - 메서드를 통해서 접근하기 때문에, 메서드 안에서 매개변수같이 어떤 올바르지 않은 입력에 대해 사전에 처리할 수 있게 제한하거나 조절할 수 있기 때문 
- 클래스의 다중 상속이 불가한 이유?
	- 여러 클래스를 다중 상속 후 2개 이상의 수퍼 클래스에 존재하는 같은 이름의 메소드 사용 시 모호함
